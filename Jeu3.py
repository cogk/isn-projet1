...
def jouer():
    global jeu_en_cours
    jeu_en_cours = True
jeu_en_cours = False # Le jeu commence quand la fonction `jouer` est appelée (bouton ou clic).
...
Label_vies = Label(fen, text='Vies: %i' % joueur_vies_restantes, fg="#3f51b5", font=(None, 20)) # On crée un Label qui affichera le nombre de vies.
...
def nouvelle_partie():
    ...
    joueur_vies_restantes = 3
    jeu_en_cours = False
    Label_vies.config(text='Vies: %i' % joueur_vies_restantes) # On affiche le nombre de vies restantes du joueur.
    Bouton_rejouer.pack_forget() # On masque le bouton Rejouer.
    Label_perdu.pack_forget() # On masque le Label de défaite.
    Label_gagné.pack_forget() # On masque le Label de victoire.

    balle_direction = (random() - 0.5) * 90 # On donne une direction aléatoire à la balle.
    x, y = balle_x_initial, balle_y_initial
    fond.coords(balle, (x, y, x + 2*balle_rayon, y + 2*balle_rayon))

    créer_briques() # On crée les briques pour pouvoir (re)jouer.

# On crée les boutons et Labels nécessaires.
...
def partie_perdue():
    Bouton_rejouer.pack()
    Label_perdu.pack(side = BOTTOM) # On place le Label au bas de la fenêtre.

def partie_gagnée():
    Bouton_rejouer.pack()
    Label_gagné.pack(side = BOTTOM) # On place le Label au bas de la fenêtre.

# On définit le rayon et les coordonnées initiales de notre balle.
balle_rayon = 15
balle_x_initial = largeur_jeu / 2
balle_y_initial = hauteur_jeu - 100

# Vecteur vitesse en coordonnées polaires.
balle_vitesse = 0.5 # vitesse
balle_direction = 0 # définie dans `nouvelle_partie`
...
# On definit la couleur des briques par leur dureté.
...
def déplacer_balle(dt):
    ...
    if y > hauteur_jeu - balle_rayon:
        ...
        Label_vies.config(text='Vies: %d' % joueur_vies_restantes)#on change le nombre de vie du joueur
